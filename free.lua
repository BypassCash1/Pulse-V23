...
function ModernUI.CreateWindow(options)
    ...
    if isTablet or isMobile then
        -- Mobile/Tablet sizing - narrower and smaller
        defaultWidth = math.floor(ViewportSize.X * 0.70)
        defaultHeight = math.floor((ViewportSize.Y - GuiInset) * 0.80)
    else
    ...
    ControlsContainer.Size = UDim2.new(0, 90, 0, 26)
    ControlsContainer.Position = UDim2.new(1, -98, 0.5, -13)
    ...
    local oldPosition = MainFrame.Position
    ...
    MinimizeButton.MouseButton1Click:Connect(function()
        ...
        if Minimized then
            ...
            if Maximized then
                Tween(MainFrame, {Position = UDim2.new(0, 0, 0, 0)}, 0.3)
            else
    ...
    -- Simple click to toggle (no dragging)
    ...
    Camera:GetPropertyChangedSignal("ViewportSize"):Connect(function()
        ...
        if isTablet or isMobile then
            -- Mobile/Tablet - narrower and smaller
            newWidth = math.floor(newViewportSize.X * 0.70)
            newHeight = math.floor((newViewportSize.Y - GuiInset) * 0.80)
        else
            -- PC - keep fixed size
            newWidth = 600
            newHeight = 450
    ...
    -- Player Info Box - fixed at bottom of MainFrame
    local PlayerInfo = Instance.new("Frame")
    PlayerInfo.Name = "PlayerInfo"
    PlayerInfo.Size = UDim2.new(0, 180, 0, 60)
    PlayerInfo.Position = UDim2.new(0, 10, 1, -70)
    ...
    PlayerInfoStroke.Transparency = 0.7
    ...
    PlayerAvatar.Position = UDim2.new(0, 8, 0, 8)
    ...
    -- Display Name Label (top line)
    local PlayerDisplayName = Instance.new("TextLabel")
    PlayerDisplayName.Name = "DisplayName"
    PlayerDisplayName.Size = UDim2.new(1, -63, 0, 20)
    PlayerDisplayName.Position = UDim2.new(0, 58, 0, 8)
    ...
    PlayerUsername.Size = UDim2.new(1, -63, 0, 18)
    PlayerUsername.Position = UDim2.new(0, 58, 0, 30)
    PlayerUsername.BackgroundTransparency = 1
    PlayerUsername.Text = "@" .. LocalPlayer.Name
    PlayerUsername.TextColor3 = Theme.SubText
    ...
    TabContainer.Size = UDim2.new(0, 180, 1, -140)
    ...
    -- Add player info to MainFrame at the bottom
    ...
    -- Hide Username Function
    function Window:HideUsername(hide)
        if hide then
            PlayerDisplayName.Text = "Hidden User"
            PlayerUsername.Text = "@***********"
        else
            PlayerDisplayName.Text = LocalPlayer.DisplayName
            PlayerUsername.Text = "@" .. LocalPlayer.Name
    ...
    -- Get HWID for unique config per machine
    ...
    local function AutoSave()
        task.spawn(function()
            pcall(function()
                ...
                writefile("sev_config_" .. HWID .. ".json", encoded)
            end)
        end)
    end
    
    function Window:SaveConfig(configName)
        configName = configName or HWID
        local success, err = pcall(function()
            local encoded = HttpService:JSONEncode(ConfigValues)
            writefile("sev_config_" .. configName .. ".json", encoded)
        end)
        return success
    end
    ...
    function Window:LoadConfig(configName)
        configName = configName or HWID
        local success, err = pcall(function()
            local fileName = "sev_config_" .. configName .. ".json"
    ...
    function Window:ReloadUI()
        -- Trigger UI reload by destroying and recreating
        if getgenv().ReloadSevUI then
            ScreenGui:Destroy()
            task.wait(0.5)
            getgenv().ReloadSevUI()
        end
        ...
        pcall(function()
            if listfiles then
                ...
                for _, file in ipairs(files) do
                    if file:match("sev_config_(.+)%.json") then
                        local configName = file:match("sev_config_(.+)%.json")
    ...
    function Window:RegisterConfigValue(key, defaultValue, callback)
        ConfigValues[key] = defaultValue
        if callback then
            getgenv().ConfigCallbacks = getgenv().ConfigCallbacks or {}
            getgenv().ConfigCallbacks[key] = callback
        end
    end
    ...
    task.spawn(function()
        task.wait(1)
        local configToLoad = HWID
        
        -- Check if user requested specific config to load
        if getgenv().LoadConfigOnStart then
            configToLoad = getgenv().LoadConfigOnStart
            getgenv().LoadConfigOnStart = nil -- Clear it after reading
        end
        
        local fileName = "sev_config_" .. configToLoad .. ".json"
        if isfile and isfile(fileName) then
            -- Set loading flag to suppress all notifications
            getgenv().LoadingConfig = true
            ...
            pcall(function()
                ...
                for key, value in pairs(decoded) do
                    ConfigValues[key] = value
                    if getgenv().ConfigCallbacks and getgenv().ConfigCallbacks[key] then
                        getgenv().ConfigCallbacks[key](value)
                    end
                end
            end)
